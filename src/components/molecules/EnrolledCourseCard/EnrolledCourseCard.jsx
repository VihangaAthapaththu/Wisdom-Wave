...
export function EnrolledCourseCard({ course }) {
  return (
    <Card className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 p-5 bg-white border border-gray-100 shadow-sm hover:shadow-md hover:shadow-[#FFA500]/5 hover:-translate-y-0.5 transition-all duration-300 rounded-2xl">
      ...
      <div className="flex items-center gap-3 sm:w-48 lg:w-64">
        <Progress value={course.progress} className="h-2 flex-1 bg-gray-100 [&>div]:bg-gradient-to-r [&>div]:from-[#FFA500] [&>div]:to-[#ff8c00] [&>div]:rounded-full" />
        <span className="font-bold text-[#FFA500] text-sm min-w-[42px] text-right">{course.progress}%</span>
      </div>
    </Card>
  );
}
